...
1. **Mobile app** (React Native / Expo) — beneficiaries & donors. Mostly reads (portfolio, services catalog, providers), plus writes: **service bookings** (guest or account) and **donations**.
...
- **donations** — `id, donor_name, user_id (nullable), cause, amount, method, recurring, status, created_at`
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/donations` | Create a donation (cause, amount, method, recurring). Returns pending/paid status |
...
| GET | `/me` · `/me/bookings` · `/me/donations` · `/me/favorites` | Profile data |
...
## 10. Notifications
- **FCM push** on: booking received, confirmed, rescheduled, cancelled; reminder N hours before appointment (cron job).
- **SMS/OTP** (optional): verification + optional booking confirmation text.
- Store device tokens per user; no-op gracefully when a channel isn't configured.
...
`serviceCategories, providers, services, governorates, bookingFormSchema, adminBookings, adminUsers, adminRoles, permissionModules, activityLog, portfolioItems, cases, projects, consultants, donations, foundationStats`.
...
6. **Portfolio CMS + Users + Roles/Activity**.
7. **Reports/analytics + exports**.
8. **Notifications (FCM) + reminders cron**.
9. **Donations** endpoints (+ gateway sandbox).
10. **Hardening**: rate limits, load test the booking endpoint (Offer §11), OWASP pass.
